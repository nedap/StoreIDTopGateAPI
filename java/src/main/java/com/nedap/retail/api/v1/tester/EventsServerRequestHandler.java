    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        switch (this.mode) {
            case RAW:
                ...
                break;
            case EPCCOUNT:
                if (event.type.equals("rfid.tag.arrive")) {
                    ...
                    for (EventEpc epc : event.epcList) {
                        EpcCounter.addEpcWithDirection(epc.epc, event.direction);
                    }
                    System.out.print("Direction " + event.direction + ": " + EpcCounter.countForDirection(event.direction) + " unique EPCs total,");
                    System.out.println(event.epcList.size() + " EPCs in this event");
                }
